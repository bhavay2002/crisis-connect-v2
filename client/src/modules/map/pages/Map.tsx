...
import {
  Flame, Droplet, Mountain, Wind, Car, AlertTriangle, MapPin, Calendar,
  ThumbsUp, ShieldCheck, Biohazard, Construction, Waves, Zap, Droplets,
  Activity, Layers, SlidersHorizontal, X,
} from "lucide-react";
...
const createColoredIcon = (color: string, isSelected = false) => {
  ...
  const svg = `<svg width="${size}" height="${Math.round(size * 1.64)}" viewBox="0 0 ${size} ${Math.round(size * 1.64)}" xmlns="http://www.w3.org/2000/svg">
    <path d="M${size / 2} 0C${size * 0.224} 0 0 ${size * 0.224} 0 ${size / 2}c0 ${size * 0.336} ${size / 2} ${size * 1.14} ${size / 2} ${size * 1.14}S${size} ${size * 0.836} ${size} ${size / 2}C${size} ${size * 0.224} ${size * 0.776} 0 ${size / 2} 0z" fill="${color}" ${isSelected ? `stroke="white" stroke-width="2"` : ""}/>
...
const typeIcons: Record<string, any> = {
  fire: Flame, flood: Droplet, earthquake: Mountain, storm: Wind,
  road_accident: Car, epidemic: Biohazard, landslide: Waves,
  gas_leak: AlertTriangle, building_collapse: Construction,
  chemical_spill: Biohazard, power_outage: Zap,
  water_contamination: Droplets, other: AlertTriangle,
};
...
// Recenter map when incident selected
...
export default function Map() {
  ...
  const filteredReports = useMemo(() => {
    ...
    if (severityFilter !== "all") f = f.filter(r => r.severity === severityFilter);
    if (timelineEnabled) {
  ...
  const mapCenter: LatLngExpression = useMemo(() => {
    const lats = filteredReports.map(r => parseFloat(r.latitude!)).filter(x => !isNaN(x));
    const lngs = filteredReports.map(r => parseFloat(r.longitude!)).filter(x => !isNaN(x));
  ...
  const heatmapPoints: [number,number,number][] = useMemo(() => {
    ...
    if (heatmapDataSource === "all" || heatmapDataSource === "reports") {
      filteredReports.forEach(r => {
        ...
        if (!isNaN(lat) && !isNaN(lng)) {
          const i = r.severity === "critical" ? 1.0 : r.severity === "high" ? 0.7 : r.severity === "medium" ? 0.5 : 0.3;
          pts.push([lat, lng, i]);
        }
      });
    }
    ...
    if (heatmapDataSource === "all" || heatmapDataSource === "resources") {
      resourceRequests.filter(r => r.status === "pending").forEach(r => {
        const lat = parseFloat(r.latitude!), lng = parseFloat(r.longitude!);
        if (!isNaN(lat) && !isNaN(lng)) {
          const i = r.urgency === "critical" ? 0.9 : r.urgency === "high" ? 0.6 : r.urgency === "medium" ? 0.4 : 0.2;
          pts.push([lat, lng, i]);
        }
      });
    }
  ...
  const flyToCenter: LatLngExpression | null = selectedIncident
    ? [selectedIncident.lat, selectedIncident.lng]
    : null;

  const handleMarkerClick = (report: DisasterReport) => {
  ...
  const activeFilters = [typeFilter !== "all", severityFilter !== "all", timeFilter !== "all"].filter(Boolean).length;
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex items-center gap-3 px-4 py-2.5 border-b bg-slate-950 flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <Button
            variant={timelineEnabled ? "secondary" : "outline"}
            size="sm"
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="flex-1 relative">
          {isLoading ? (
            ...
            >
              ...
              {evacuationZonesEnabled && demoEvacuationZones.map((zone, i) => (
              ...
              {roadsEnabled && demoMajorRoads.map((r, i) => (
              ...
              {!heatmapEnabled && filteredReports.map(report => {
                ...
                const icon = createColoredIcon(severityColors[report.severity] || "#64748b", isSelected);
                return (
                  <Marker
                    key={report.id}
                    position={[lat, lng]}
                    ...
                    <Popup>
                      <div className="min-w-[180px]">
                        ...
                        <Button size="sm" className="w-full h-7 text-xs"
                          onClick={() => handleMarkerClick(report)} data-testid={`button-view-details-${report.id}`}>
                          Open Command Panel
                        </Button>
                      </div>
                    </Popup>
        ...
        {/* Incident Panel */}
        <IncidentPanel onVerify={handleVerify} />
      </div>
    </div>
  );
}
